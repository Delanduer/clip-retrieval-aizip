...
def clip_back(
    ...
    from clip_retrieval.clip_front import add_static_endpoints  # pylint: disable=import-outside-toplevel
...
class MetadataService(fastResource):
    """The metadata service provides metadata given indices"""
...
def clip_back_fastapi(
    ...
    #app.add_middleware
    indices = IndicesList(indices=list([index_folder]))
    api.add_resource(indices, "/indices-list")
    metadata = MetadataService(clip_resources=clip_resources)
